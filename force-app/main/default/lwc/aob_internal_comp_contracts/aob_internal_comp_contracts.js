/**
 * @description       : Displays a list of contracts relating to an application
...
export default class aob_internal_comp_contracts extends NavigationMixin(LightningElement) {
    showContracts=false;
    signPage=false;
    url;
    showFrame=false;
    ...
    columns = [
        { label: 'Contract Name', fieldName: 'value' },
        {
            label:'Preview',
            type: 'button',
            initialWidth: 135,
    ...
    fetchDocumentDownload(key,fileName) {
        this.isLoaded = false;
        getDocument({
            'applicationId': this.applicationId,
            'itemId':key
    ...
    handleRowAction(event) {
        ...
        if (actionName === 'preview') {
            this.fetchDocumentPreview(row.key);
        } else if (actionName === 'download') {
            this.fetchDocumentDownload(row.key,row.value);
        }
      }
    onBack(){
    ...
    onLawTrust(){
        window.open(
            'https://signinghub.standardbank.co.za/',
            '_blank'
          );
    }
